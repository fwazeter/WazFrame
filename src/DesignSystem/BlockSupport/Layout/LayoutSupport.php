...
class LayoutSupport implements BlockSupportInterface {
	...
	public function render ( string $block_content, array $block ): string {
		...
		// either pulls the block's custom layout properties, or sets default. ?? means if expr1 exists & is not
		// NULL then expr1, if expr1 does not exist or is NULL, value is expr2. Original = isset(
		// $block[$attrs]['layout'] ) ? $block['attrs']['layout'] : $block_default_layout
		$block_layout_value = $block['attrs']['layout'] ?? $block_default_layout;
